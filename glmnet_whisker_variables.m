...
%         lickBoutInterval = 1; % licks separated by 1 s regarded as different licking bouts
        ...
        savefnResult = sprintf('glmWithWhiskerTouchVariables_JK%03dS%02d',mouse, session); % m(n) meaining method(n)
...
%                             tempAbsDPhi = zeros(1,length(time)-1);
                ...
                whiskerTouchInd = max(lickInd) + 1 : max(lickInd) + size(whiskerTouchMat,2);
