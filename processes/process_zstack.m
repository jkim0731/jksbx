...
%     draw_C2(zstackFn(1).name)
    tempFlist = dir(['F_', s2pList{i}, '_plane*_proc_final.mat']); % assuming everything is processed already
...
for i = 1 : length(mice)
    ...
    for j = 1 : 8
        ...
        for k = 1 : length(flist)
            load(flist(k).name) % loading dat and spk
