...
%     for si = 1 : length(sessions{mi})
    for si = 1
        ...
        savefn = [u.mouseName,u.sessionName,'singleCell_Z_ANOVA.mat'];
        ...
        % for confirmation
        Ztotal = cell(length(u.cellNums),1); % for all cells. ordered by cell number
        ...
        %     cellid = ci;
            ...
            % turn into Z value
            % first, calculate baseline std
            wholeF = cell2mat(cellfun(@(x) x,dF, 'uniformoutput', false));
            eachLength = cellfun(@(x) size(x,1), dF);
            baseline = wholeF(1:3,:);
            s = std(baseline(:));            
            ...
            z = cell(length(dF),1);
            for zi = 1 : length(z)
                z{zi} = wholeZ(sum(eachLength(1:zi-1))+1 : sum(eachLength(1:zi)),:);
            end
            
            Ztotal{cellid} = z;

            % ANOVA
            touchNumGroups = cellfun(@(x) size(x,1), z);
            timeAverageZ = zeros(sum(touchNumGroups),1);
            anovaGroupsF = zeros(sum(cellfun(@(x) size(x,1), z)),1);
            for ai = 1 : length(angles)
                timeAverageZ( sum(touchNumGroups(1:(ai-1))) + 1 : sum(touchNumGroups(1:ai)) ) = mean(z{ai}(:,baseFrameNum+1:end),2);
                anovaGroupsF( sum(touchNumGroups(1:(ai-1))) + 1 : sum(touchNumGroups(1:ai)) ) = deal(ai);
            end

            [anovaP, anovaTable, anovaStat] = anova1(timeAverageZ, anovaGroupsF, 'off');
            ...
            if anovaP <= thresholdAnova        
                ...
                for ai = 1 : length(angles)
                    tTestH(ai) = ttest(timeAverageZ( sum(touchNumGroups(1:(ai-1))) + 1 : sum(touchNumGroups(1:ai)) ));
                end
                ...
                if isempty(tTestInd) % happens sometimes, when the response level is low
                    ...
                    ztuned = z{tunedAngleInd}(:,baseFrameNum+1 : end); % for readability
                    responseNum = 0;
                    for ri = 1:size(ztuned,1)
                        tempTS = ztuned(ri,:);
                        [~, p] = ttest(tempTS');
                    ...
                    template = mean(ztuned);
                    rho = zeros(size(ztuned,1),1);
                    for ri = 1 : length(rho)
                        tempTS = ztuned(ri,:);
                ...
                responseH = ttest(timeAverageZ);
                if responseH
                    cellsNTResponse = [cellsNTResponse; cellNum];
                    if mean(timeAverageZ) > 0
                        NTRdirection = [NTRdirection; 1]; % increase
                    elseif mean(timeAverageZ) < 0
                    ...
                    % Response probability
                    Zall = zeros(sum(cellfun(@(x) size(x,1), z)), afterFrameNum); % for readability
                    for ai = 1 : length(angles)
                        Zall( sum(touchNumGroups(1:(ai-1))) + 1 : sum(touchNumGroups(1:ai)), : ) = z{ai}(:,baseFrameNum+1 : end);
                    end
                    responseNum = 0;
                    for ri = 1 : size(Zall,1)
                        tempTS = Zall(ri,:);
                        [~, p] = ttest(tempTS');
                        if p <= thresholdTtestResponse
                            if mean(timeAverageZ) * mean(tempTS) > 0
                                responseNum = responseNum + 1;
                            end
                        end
                    end
        ...
        save(savefn, 'cells*','tune*','NTR*','Ztotal')
        end
    end
end