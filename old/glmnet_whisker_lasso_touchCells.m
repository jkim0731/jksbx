...
% Adding more touch variables (instantaneous features) + absolute vertical
...
% Keeping it with 12 features (6 kinetics 6 kinematics)
% Mean instead of max value in each frame
% Calcuate kinematics at the onset, not -1 frame
% 2019/09/23 JK
...
%     for si = 1:length(sessions{mi})
    for si = 2
        ...
        for ri = startRepetition : repetition % repetition index
                ...
                allPredictorsRaw = cell(8,1); % to save for later use
...
%%
            save(sprintf('%s_R%02d',savefnResult, ri), 'fit*', 'allPredictors', 'allPredictorsRaw','indPartial', '*Group', 'testTn', 'trainingTn', 'lambdaCV', '*Opt', 'done', 'pThreshold*', '*Shift', 'cellTime', 'cIDAll', 'ratio*');
