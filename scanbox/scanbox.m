...
% Last Modified by GUIDE v2.5 21-May-2018 12:43:12
...
if isfield(sbconfig, 'speedbelt_com') && ~isempty(sbconfig.speedbelt_com)
...
global opto2pow_list linkpmt1_list linkpmt0_list

opto2pow_list = cell(3,1);
linkpmt1_list = cell(3,1);
linkpmt0_list = cell(3,1);
...
cprintf('comment','[%02d] Setting Line2 as trigger source for all DALSA cameras (please wait)\n',istep); istep=istep+1;
...
%   This is not optimal. Use separate code dealing with mmemmap 2018/05/15 JK
...
if(sbconfig.tri_knob_ver>1)
    panel_list = { {handles.realtime} {handles.knobby_sched} {handles.sspanel} {handles.message_panel}  {handles.notes} {handles.optogenetics_panel} {handles.intrinsic_panel} {handles.JK} };
else
...
if saveData
    ...
    fn = [datadir filesep animal filesep sprintf('%s_%s_%03d.sbx',animal,unit,experiment)];
    ...
    if(sbconfig.ephys)
        global efid
        fn = [datadir filesep animal filesep sprintf('%s_%s_%03d.ephys',animal,unit,experiment)];
        efid = fopen(fn,'w');
    end
...
global roipix
global otwave otparam otwave_um opto2pow opto2pow_list linkpmt0_list linkpmt1_list % JK
...
if strcmp(handles.depth_find_button.String, 'Find Ref Matching Depth') % want to make the figure alive while finding FOV depth 2018/05/21 JK
...
if isfield(sbconfig, 'speedbelt_com') && (~isempty(sbconfig.speedbelt_com) && (handles.sbcheck.Value>0))
...
if(mm_flag)
    ...
    mmfile.Data.header(8) = sum(int8(unit)); % I want the unit to be string. This is temporary solution, hoping that nobody will use this header information in memmap. 2018/05/18 JK
...
% Remove all knobby_enable related codes because it is a piece of shit.
% if(sbconfig.tri_knob_ver>1)
%     
%     tri_send('KBY',0,81,150,0);
%     handles.knobby_enable.Value = 0;
...
if(fid ~= -1)
    if(ttlonline && ~isempty(trial_acc))
        fn = sprintf('%s\\%s\\%s_%s_%03d_trials.mat',datadir,animal,animal,unit,experiment);
...
if(fid ~= -1)
    if(nroi>0)
        fn = sprintf('%s\\%s\\%s_%s_%03d_realtime.mat',datadir,animal,animal,unit,experiment);
        oldstr = get(handles.etime,'String');
...
if(fid ~= -1)
    if(get(handles.stabilize,'Value'))
        fn = sprintf('%s\\%s\\%s_%s_%03d_align.mat',datadir,animal,animal,unit,experiment);
        oldstr = get(handles.etime,'String');
...
if(fid ~= -1)
    if(quad_flag)
        fn = sprintf('%s\\%s\\%s_%s_%03d_quadrature.mat',datadir,animal,animal,unit,experiment);
        oldstr = get(handles.etime,'String');
    ...
    if(speedbelt_flag)
        fn = sprintf('%s\\%s\\%s_%s_%03d_speedbelt.mat',datadir,animal,animal,unit,experiment);
        oldstr = get(handles.etime,'String');
...
if(fid ~= -1)
    ...
    if(get(handles.wc,'Value') || get(handles.eyet,'Value'))
        ...
        if(get(handles.wc,'Value')) % write wcam data...
            ...
            fn = sprintf('%s\\%s\\%s_%s_%03d_ball.mat',datadir,animal,animal,unit,experiment);
            flushdata(wcam);
        ...
        if(get(handles.eyet,'Value')) % write eyet data...
            ...
            fn = sprintf('%s\\%s\\%s_%s_%03d_eye.mat',datadir,animal,animal,unit,experiment);
            flushdata(eyecam);
            save(fn,'data','time','abstime','-v7.3');
            clear data time abstime;
        end
...
if fid ~= -1
    fclose(fid);
    fid = -1;
    fn = [datadir filesep animal filesep sprintf('%s_%s_%03d.mat',animal,unit,experiment)];
    ...
    info.opto2pow_list = opto2pow_list; % JK
    info.linkpmt1_list = linkpmt1_list; % JK
    info.linkpmt0_list = linkpmt0_list; % JK
    info.deadband = sbconfig.deadband; % JK
...
cd([datadir filesep animal]); % JK 2017/10/01
...
switch(s(1))
    ...
    case 'L'                % programatically turn laser ON/OFF
        global org_pock
        h = findobj(scanbox_h,'Tag','linkcheck');
        val = get(h,'Value');
        if(s(2)~='0')
            if val == 1 % 2017/10/05 JK laser not blanking when using optotune power link            
                sb_pockels(0,0);
                sb_current_power_active(1); 
            else
                sb_pockels(org_pock(1),org_pock(2))            
            end
        else
            if val == 1
                sb_current_power_active(0); % 2017/10/05 JK laser not blanking when using optotune power link
            end
            sb_pockels(0,0)
        end
    ...
    case 'R'        % Recall 2017/09/30 JK
        str = s(2);        
        h = findobj(scanbox_h,'Tag',['recall', str]); % str should be a, b, or c (lower class)
        f = get(h,'Callback');
        f(h,guidata(h));      
        
    case 'I'        % Load l"I"nk 2017/09/30 JK
        str = s(2);
        h = findobj(scanbox_h,'Tag',['LoadLink', str]); % str should be A, B, or C (upper class)
        f = get(h,'Callback');
        f(h,guidata(h));
        
    case 't'        % set transmission value 2017/11/02 JK
        val = str2double(s(2:end));
        h = findobj(scanbox_h,'Tag','pockval');
        h.Value = round(val/100*255.0);
        sb_pockels(0,uint8(h.Value));
        h = findobj(scanbox_h,'Tag','powertxt');
        h.String = sprintf('%3d%%',round(val));
        
    case '0'        % set pmt0 value 2017/11/02 Jk
        val = str2double(s(2:end));
        h = findobj(scanbox_h,'Tag','pmt0');
        h.Value = val;        
        sb_gain0(uint8(255*val));
        h = findobj(scanbox_h,'Tag','pmt0txt');
        h.String = sprintf('%1.2f',val);
...
handles.linktext.String = '';   % Clear link usage in JKpanel 2107/10/01
...
% Ignore all knobby_enable thing because it is fucked up
% global sbconfig
% 
% if sbconfig.tri_knob_ver>1
%     
...
% This does not work (stops at while loop in tri_send('KBY ... during
% h.grabb.callback 2018/05/09 JK
...
global z_stack_bottom % 2017/11/04 JK for z-stack imaging
handles.text_bottom.String = 'Bottom (z) = '; % 2017/11/04 JK for z-stack imaging
z_stack_bottom = []; % 2017/11/04 JK for z-stack imaging
...
global z_stack_top % 2017/11/04 JK for z-stack imaging
handles.text_top.String = 'Top (z) = '; % 2017/11/04 JK for z-stack imaging
z_stack_top = []; % 2017/11/04 JK for z-stack imaging
...
% --- Executes on button press in SaveLinkA.
function SaveLinkA_Callback(hObject, eventdata, handles)
% hObject    handle to SaveLinkA (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

global opto2pow opto2pow_list linkpmt0_list linkpmt1_list
if ~isempty(opto2pow)
    opto2pow_list{1} = opto2pow;
    handles.linktext.String = 'Current link saved to A';

    linkpmt0_list{1} = handles.pmt0.Value;
    linkpmt1_list{1} = handles.pmt1.Value;
else
    handles.linktext.String = 'Empty. Link first.';
end

% --- Executes on button press in LoadLinkA.
function LoadLinkA_Callback(hObject, eventdata, handles)
% hObject    handle to LoadLinkA (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

global opto2pow opto2pow_list linkpmt0_list linkpmt1_list
if ~isempty(opto2pow_list{1})
    opto2pow = opto2pow_list{1};
    handles.linktext.String = 'Using Link A';

    handles.linkcheck.Value = 1;
    handles.linkcheck.Callback(handles.linkcheck,handles);

    handles.pmt0.Value = linkpmt0_list{1};
    sb_gain0(uint8(255*linkpmt0_list{1}));
    handles.pmt0txt.String = sprintf('%1.2f',linkpmt0_list{1});
    handles.pmt1.Value = linkpmt1_list{1};
    sb_gain1(uint8(255*linkpmt1_list{1}));
    handles.pmt1txt.String = sprintf('%1.2f',linkpmt1_list{1});
else
    handles.linktext.String = 'No Link A. Save one first';
end


% --- Executes on button press in SaveLinkB.
function SaveLinkB_Callback(hObject, eventdata, handles)	
% hObject    handle to SaveLinkB (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

global opto2pow opto2pow_list linkpmt0_list linkpmt1_list
if ~isempty(opto2pow)
    opto2pow_list{2} = opto2pow;
    handles.linktext.String = 'Current link saved to B';

    linkpmt0_list{2} = handles.pmt0.Value;
    linkpmt1_list{2} = handles.pmt1.Value;
else
    handles.linktext.String = 'Empty. Link first.';
end


% --- Executes on button press in LoadLinkB.
function LoadLinkB_Callback(hObject, eventdata, handles)
% hObject    handle to LoadLinkB (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

global opto2pow opto2pow_list linkpmt0_list linkpmt1_list
if ~isempty(opto2pow_list{2})
    opto2pow = opto2pow_list{2};
    handles.linktext.String = 'Using Link B';

    handles.linkcheck.Value = 1;
    handles.linkcheck.Callback(handles.linkcheck,handles);

    handles.pmt0.Value = linkpmt0_list{2};
    sb_gain0(uint8(255*linkpmt0_list{2}));
    handles.pmt0txt.String = sprintf('%1.2f',linkpmt0_list{2});
    handles.pmt1.Value = linkpmt1_list{2};
    sb_gain1(uint8(255*linkpmt1_list{2}));
    handles.pmt1txt.String = sprintf('%1.2f',linkpmt1_list{2});
else
    handles.linktext.String = 'No Link B. Save one first';
end


% --- Executes on button press in SaveLinkC.
function SaveLinkC_Callback(hObject, eventdata, handles)
% hObject    handle to SaveLinkC (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

global opto2pow opto2pow_list linkpmt0_list linkpmt1_list
if ~isempty(opto2pow)
    opto2pow_list{3} = opto2pow;
    handles.linktext.String = 'Current link saved to C';

    linkpmt0_list{3} = handles.pmt0.Value;
    linkpmt1_list{3} = handles.pmt1.Value;
else
    handles.linktext.String = 'Empty. Link first.';
end


% --- Executes on button press in LoadLinkC.
function LoadLinkC_Callback(hObject, eventdata, handles)
% hObject    handle to LoadLinkC (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

global opto2pow opto2pow_list linkpmt0_list linkpmt1_list
if ~isempty(opto2pow_list{3})
    opto2pow = opto2pow_list{3};
    handles.linktext.String = 'Using Link C';

    handles.linkcheck.Value = 1;
    handles.linkcheck.Callback(handles.linkcheck,handles);

    handles.pmt0.Value = linkpmt0_list{3};
    sb_gain0(uint8(255*linkpmt0_list{3}));
    handles.pmt0txt.String = sprintf('%1.2f',linkpmt0_list{3});
    handles.pmt1.Value = linkpmt1_list{3};
    sb_gain1(uint8(255*linkpmt1_list{3}));
    handles.pmt1txt.String = sprintf('%1.2f',linkpmt1_list{3});
else
    handles.linktext.String = 'No Link C. Save one first';
end


% --- Executes on button press in open_ref.
function open_ref_Callback(hObject, eventdata, handles)
% hObject    handle to open_ref (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
if ~isempty(ref_dir) && ~isempty(ref_file)
    ...
    if strcmp(ref_dir(end),filesep)
        ref_dir = ref_dir(1:end-1);
    end
    flipRefDir = flip(ref_dir);
    [flipAnimal,flipDir] = strtok(flipRefDir,filesep);
    animal = flip(flipAnimal);
    handles.animal.String = animal;
    datadir = flip(flipDir);
    handles.dirname.String = datadir;
end

% --- Executes on button press in depth_find_button.
function depth_find_button_Callback(hObject, eventdata, handles)
% hObject    handle to depth_find_button (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

global datadir ref_file ref_dir animal experiment unit roi_traces_h img0_h
stacknum = 11;
firstStep = 3;
secondStep = 1;

refCh = handles.RefCh.Value; 
switch get(hObject,'String')
    case 'Find Ref Matching Depth'
        ...
        if refCh == 1
            if iscell(m1)
                if length(m1) == 1 || handles.volumePosition.Value == 1
                    refIm = m1{1};
                elseif handles.volumePosition.Value == 2
                    refIm = m1{5};
                else
                    refIm = m1{end};
                end
        ...
        elseif refCh == 2
            if iscell(m2)
                if length(m2) == 1 || handles.volumePosition.Value == 1
                    refIm = m2{1};
                elseif handles.volumePosition.Value == 2
                    refIm = m2{5};
                else
        ...
        if handles.FOVmatchStep.Value ~= 1 % 2-step automatic search. Takes ~ 10 min           
            ...
            secondStep = 0;
            if handles.FOVmatchRange.Value ~= 1
                stacknum = str2double(handles.FOVmatchRange.String{handles.FOVmatchRange.Value})*2+1;
            end
        end
        
        % move objective lens -30 um
        for i = 1 : round((stacknum-1)/2)
            tri_send('KBY',0,0,-firstStep); pause(1)
        end
        %tri_send very unreliable. Panicking frequently. Have at least 2
        %second pause for every 'KBY',0,0 command.
        % simple numeric summation does not match to z value. 

        % take z-stack in 3 um resolution for 60 um (21 planes)
        currAnimal = animal;        
        currUnit = unit;     
        currExp = experiment;

        handles.frames.String = '100';
        animal = 'xx0'; unit = '000'; experiment = 0;
        handles.animal.String = animal; handles.unit.String = unit; handles.expt.String = sprintf('%03d',experiment);

        cd(datadir)
        if ~exist(animal,'dir')
            mkdir(datadir,animal)    
        end
        cd([datadir, filesep, animal])
        fclose all; delete('xx0_000_*') % This is safer.
        i = 0;
        while strcmp((get(hObject,'String')), 'Abort') && i < stacknum
            i = i + 1;
        ...
        if i < stacknum
            set(hObject,'String','Find Ref Matching Depth');
            animal = currAnimal; unit = currUnit; experiment = currExp;        
            handles.animal.String = animal; handles.unit.String = unit; handles.expt.String = sprintf('%03d',experiment);
            cd([datadir, filesep, animal])
            return
        end
        ...
        while strcmp((get(hObject,'String')), 'Abort') && i < stacknum
            i = i + 1;
            tri_send('KBY',0,0,-firstStep); pause(1)
        end % simple numeric summation does not match to z value. 

        % calculate the best matching plane
        jksbxaligndir('','','skip')

        fnlist = dir('xx0_000_*.align');
        corval = zeros(length(fnlist),1);
        planeInd = zeros(length(fnlist),1);
        for i = 1 : length(fnlist)
            tempname = strtok(fnlist(i).name,'.');
            fliptempname = flip(tempname);
            tempind = strtok(fliptempname,'_');
            planeInd(i) = str2double(flip(tempind)); % Just in case when dir is not sorting

            load(fnlist(i).name,'-mat'); % loading m1{}
            if refCh == 1
                currIm = m1{end};
            elseif refCh == 2
                currIm = m2{end};
            ...
            if u >= 0
                if v >= 0
                    currIm = currIm(u+1:end,v+1:end);
                    tempRef = refIm(u+1:end,v+1:end);
                else
                    currIm = currIm(u+1:end,1:end+v);
                    tempRef = refIm(u+1:end,1:end+v);
                end
            else
                if v >= 0
                    currIm = currIm(1:end+u,v+1:end);
                    tempRef = refIm(1:end+u,v+1:end);
                else
                    currIm = currIm(1:end+u,1:end+v);
                    tempRef = refIm(1:end+u,1:end+v);
                end
            end
            temprho = corrcoef(double(currIm(:)), double(tempRef(:)));            
%             [~, reg] = imregdemons(currIm, tempRef, [100 50 25],'AccumulatedFieldSmoothing',1.5,'PyramidLevels',3);
%             reg2 = reg(:);
%             tempRef2 = tempRef(:);
%             temprho = corrcoef(double(reg2(reg2>0)), double(tempRef2(reg2>0)));            
%             imshowpair(reg, tempRef, 'Parent', roi_traces_h)
            imshowpair(currIm, tempRef, 'Parent', roi_traces_h)
            corval(i) = temprho(1,2);
        end
        [planeNums,isort] = sort(planeInd,1,'ascend');
        corval = corval(isort); % correlation values sorted from bottom to top 
        [~,imax] = max(corval);        
        fh = figure;        
        subplot(121), plot(planeNums,corval, 'k-'), hold on, plot(planeNums(imax),corval(imax),'r.')

        % go to that plane
        for i = 1 : planeNums(imax)
            tri_send('KBY',0,0, firstStep); pause(1)
        end
        
        if secondStep
            % move objective lens -10 um
            for i = 1 : round((stacknum-1)/2)
                tri_send('KBY',0,0,-secondStep); pause(1)
            end        

            % take z-stack in 1 um resolution for 21 um (21 planes)
            fclose all; delete('xx0_001_*') % This is safer.
            animal = 'xx0'; unit = '001'; experiment = 0;
            handles.animal.String = animal; handles.unit.String = unit; handles.expt.String = sprintf('%03d',experiment);
...
%                 imshowpair(reg, tempRef, 'Parent', roi_traces_h)
            ...
            subplot(122), plot(planeNums,corval, 'k-'), hold on, plot(planeNums(imax),corval(imax),'r.')
    ...
    case 'Abort'
        set(hObject,'String','Find Ref Matching Depth');
        if strcmp(handles.grabb.String,'Abort')
            grabb_Callback(handles.grabb,[],handles)
        end
...
% --- Executes on selection change in FOVmatchStep.
function FOVmatchStep_Callback(hObject, eventdata, handles)
% hObject    handle to FOVmatchStep (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns FOVmatchStep contents as cell array
%        contents{get(hObject,'Value')} returns selected item from FOVmatchStep
if handles.FOVmatchStep.Value == 1
    handles.FOVmatchRange.Value = 1;
end


% --- Executes during object creation, after setting all properties.
function FOVmatchStep_CreateFcn(hObject, eventdata, handles)
% hObject    handle to FOVmatchStep (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on selection change in FOVmatchRange.
function FOVmatchRange_Callback(hObject, eventdata, handles)
% hObject    handle to FOVmatchRange (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns FOVmatchRange contents as cell array
%        contents{get(hObject,'Value')} returns selected item from FOVmatchRange
if handles.FOVmatchRange.Value == 1
    handles.FOVmatchStep.Value = 1;
end


% --- Executes during object creation, after setting all properties.
function FOVmatchRange_CreateFcn(hObject, eventdata, handles)
% hObject    handle to FOVmatchRange (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in knobby_link.
function knobby_link_Callback(hObject, eventdata, handles)
% hObject    handle to knobby_link (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of knobby_link

zpos = tri_send('KBY',0,100,0,0);
ypos = tri_send('KBY',0,101,0,0);
xpos = tri_send('KBY',0,102,0,0);

[sz1, sz2] = size(handles.knobby_table_jk.Data);
data = zeros(sz1,sz2);
if sz1 > 0
    for i = 1 : sz1
        if isempty(handles.knobby_table_jk.Data{i,1})
            data(i,:) = zeros(1,sz2);
        else
            for j = 1 : sz2
                data(i,j) = handles.knobby_table_jk.Data{i,j};
            end
        end
    end
end
pockval = handles.pockval.Value/255.0*100;
if isempty(data)
    handles.knobby_table_jk.Data = cell(1,6);
    handles.knobby_table_jk.Data{1,1} = xpos.val;
    handles.knobby_table_jk.Data{1,2} = ypos.val;
    handles.knobby_table_jk.Data{1,3} = zpos.val;
    handles.knobby_table_jk.Data{1,4} = pockval;
    handles.knobby_table_jk.Data{1,5} = handles.pmt0.Value;
    handles.knobby_table_jk.Data{1,6} = handles.pmt1.Value;
elseif min(min(data)) == 0 && max(max(data)) == 0 % which means there was no link before.
    handles.knobby_table_jk.Data = cell(1,6);
    handles.knobby_table_jk.Data{1,1} = xpos.val;
    handles.knobby_table_jk.Data{1,2} = ypos.val;
    handles.knobby_table_jk.Data{1,3} = zpos.val;
    ...
    handles.knobby_table_jk.Data{1,5} = handles.pmt0.Value;
    handles.knobby_table_jk.Data{1,6} = handles.pmt1.Value;
elseif isempty(find(data(:,3)==zpos.val,1))    
    data(end+1,:) = [xpos.val, ypos.val, zpos.val, pockval, handles.pmt0.Value, handles.pmt1.Value];
    % sort by z value
    [~,ind] = sort(data(:,3)); 
    for i = 1 : 6
        temp = data(:,i);
    ...
    handles.knobby_table_jk.Data = cell(size(data,1),6);
    for i = 1 : size(data,1)
        for j = 1 : size(data,2)
            handles.knobby_table_jk.Data{i,j} = data(i,j);
        end
    end

    drawnow;
else
    warndlg('Same position is already registered.')
end


% --- Executes on button press in knobby_clear.
function knobby_clear_Callback(hObject, eventdata, handles)
% hObject    handle to knobby_clear (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of knobby_clear
handles.knobby_table_jk.Data = cell(1,6);
handles.knobby_reference.String = '';
...
% Hint: get(hObject,'Value') returns toggle state of set_bottom
global z_stack_bottom
storea_Callback(handles.storea,[],handles);

zpos = tri_send('KBY',0,100,0,0);
z_stack_bottom = zpos.val;

handles.text_bottom.String = sprintf('Bottom (z) = %+5.2f um', zpos.val);

% --- Executes on button press in go_to_bottom.
function go_to_bottom_Callback(hObject, eventdata, handles)
% hObject    handle to go_to_bottom (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of go_to_bottom
recalla_Callback(handles.recalla,[],handles);
...
% Hint: get(hObject,'Value') returns toggle state of set_top
global z_stack_top

storeb_Callback(handles.storeb,[],handles);

zpos = tri_send('KBY',0,100,0,0);
z_stack_top = zpos.val;

handles.text_top.String = sprintf('Top (z) = %+5.2f um', zpos.val);


% --- Executes on button press in go_to_top.
function go_to_top_Callback(hObject, eventdata, handles)
% hObject    handle to go_to_top (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of go_to_top
recallb_Callback(handles.recallb,[],handles);


function frame_per_slice_Callback(hObject, eventdata, handles)
% hObject    handle to frame_per_slice (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of frame_per_slice as text
%        str2double(get(hObject,'String')) returns contents of frame_per_slice as a double


% --- Executes during object creation, after setting all properties.
function frame_per_slice_CreateFcn(hObject, eventdata, handles)
% hObject    handle to frame_per_slice (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
function interval_distance_Callback(hObject, eventdata, handles)
% hObject    handle to interval_distance (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of interval_distance as text
%        str2double(get(hObject,'String')) returns contents of interval_distance as a double


% --- Executes during object creation, after setting all properties.
function interval_distance_CreateFcn(hObject, eventdata, handles)
% hObject    handle to interval_distance (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Executes on button press in execute_zstack.
function execute_zstack_Callback(hObject, eventdata, handles)
% hObject    handle to execute_zstack (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of execute_zstack
global z_stack_bottom z_stack_top
if ~isempty(z_stack_bottom) && ~isempty(z_stack_top)
    abort_zstack = 0;
    start = 1;
    ...
    switch get(hObject,'String')
        case 'Z-stack'
            set(hObject,'String','Abort');
            while strcmp((get(hObject,'String')), 'Abort')
                if start % start from the bottom slice
                    go_to_bottom_Callback(handles.go_to_bottom, [], handles)
                    start = 0;
                else
                    if strcmp(handles.grabb.String, 'Grab') % run only when it's not imaging
                        handles.frames.String = handles.frame_per_slice.String;
                        step = round(str2double(handles.interval_distance.String));
                        tri_send('KBY', 0, 0, step)
                        pause(1)
                        zpos = tri_send('KBY',0,100,0,0);
                        table = cell2mat(handles.knobby_table_jk.Data);
                        if isempty(table)
                            pmt0val = handles.pmt0.Value;                            
                            pmt1val = handles.pmt1.Value;
                            pockval = handles.pockval.Value;
                        elseif size(handles.knobby_table_jk.Data,1) == 1
                            pmt0val = table(1,5);
                            pmt1val = table(1,6);
                            pockval = table(1,4);
                        else
                            z_table = table(:,3);
                            if ~isempty(find(z_table == zpos.val,1))
                                ind = find(z_table == zpos.val,1);
                                pmt0val = table(ind,5);
                                pmt1val = table(ind,6);
                                pockval = table(ind,4);
                            else
                                left_ind = find(z_table < zpos.val, 1, 'last');
                                right_ind = find(z_table > zpos.val, 1, 'first');
                                if isempty(left_ind)
                                    left_ind = 1;
                                end
                                if isempty(right_ind)
                                    right_ind = size(z_table,1);
                                end
                                if right_ind == left_ind
                                    pmt0val = table(left_ind,5);
                                    pmt1val = table(left_ind,6);
                                    pockval = table(left_ind,4);
                                else
                                    interp_factor = (zpos.val - z_table(left_ind)) / (z_table(right_ind) - z_table(left_ind));

                                    left_pmt0 = table(left_ind,5);
                                    right_pmt0 = table(right_ind,5);
                                    pmt0val = left_pmt0 + interp_factor * (right_pmt0 - left_pmt0);

                                    left_pmt1 = table(left_ind,6);
                                    right_pmt1 = table(right_ind,6);
                                    pmt1val = left_pmt1 + interp_factor * (right_pmt1 - left_pmt1);

                                    left_pock = table(left_ind,4);
                                    right_pock = table(right_ind,4);
                                    pockval = left_pock + interp_factor * (right_pock - left_pock);                                                            
                                end
                            end
                        end
                        handles.pmt0.Value = pmt0val;
                        pmt0_Callback(handles.pmt0, [], handles);
                        handles.pmt1.Value = pmt1val;
                        pmt1_Callback(handles.pmt1, [], handles);

                        handles.pockval.Value = pockval*255.0/100;
                        pockval_Callback(handles.pockval,[],handles);

                        if zpos.val >= z_stack_bottom && zpos.val <= z_stack_top
                            grabb_Callback(handles.grabb,[],handles)
                        else
                            msgbox('z-stack done')
                            set(hObject,'String','Z-stack');
                            return
                        end
                    end
                end
    ...
    warndlg('Set bottom and top position first.');
    return
end


% --- Executes on button press in link_from_file.
function link_from_file_Callback(hObject, eventdata, handles)
% hObject    handle to link_from_file (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global opto2pow_list linkpmt0_list linkpmt1_list
[fn,path] = uigetfile('*.mat');
current_dir = pwd;
cd(path)
load(fn)
try
    opto2pow_list = info.opto2pow_list;
    linkpmt0_list = info.linkpmt0_list;
    linkpmt1_list = info.linkpmt1_list;
catch
    errordlg('no link lists in this file')
end
cd(current_dir)
...
% global datadir animal
% [fn, ~] = uigetfile([datadir, filesep, animal, filesep, '*.mat']);
% settings = load(fn);
% handles.frames.String = settings.info.frames.String;
% handles.lines.String = settings.info.lines.String;
% handles.magnification.Value = settings.info.magnification.Value;


% --- Executes on selection change in RefCh.
function RefCh_Callback(hObject, eventdata, handles)
% hObject    handle to RefCh (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns RefCh contents as cell array
%        contents{get(hObject,'Value')} returns selected item from RefCh
...
% --- Executes on selection change in volumePosition.
function volumePosition_Callback(hObject, eventdata, handles)
% hObject    handle to volumePosition (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns volumePosition contents as cell array
%        contents{get(hObject,'Value')} returns selected item from volumePosition


% --- Executes during object creation, after setting all properties.
function volumePosition_CreateFcn(hObject, eventdata, handles)
% hObject    handle to volumePosition (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
...
% --- Otherwise, executes on mouse press in 5 pixel border or over volumePosition.
function volumePosition_ButtonDownFcn(hObject, eventdata, handles)
% hObject    handle to volumePosition (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
