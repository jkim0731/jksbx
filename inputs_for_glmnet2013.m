...
%     for si = 1
        ...
        %     for plane = 1
                ...
                pTouchDuration = cell2mat(cellfun(@(x) [zeros(1,posShift), histcounts2013(cell2mat(cellfun(@(y) y', x.protractionTouchChunks, 'uniformoutput', false)) * whiskerVideoFrameDuration, [0, x.tpmTime{plane}]), zeros(1,posShift)], ...
                    u.trials(tind)','uniformoutput',false));
                rTouchDuration = cell2mat(cellfun(@(x) [zeros(1,posShift), histcounts2013(cell2mat(cellfun(@(y) y', x.retractionTouchChunks, 'uniformoutput', false)) * whiskerVideoFrameDuration, [0, x.tpmTime{plane}]), zeros(1,posShift)], ...
                    u.trials(tind)','uniformoutput',false));
                ...
                lLickOnset = cell2mat(cellfun(@(x) [zeros(1,posShift), histcounts2013(x.leftLickOnset, [0, x.tpmTime{plane}]), zeros(1,posShift)], v(tind)','uniformoutput',false));
                rLickOnset = cell2mat(cellfun(@(x) [zeros(1,posShift), histcounts2013(x.rightLickOnset, [0, x.tpmTime{plane}]), zeros(1,posShift)], v(tind)','uniformoutput',false));
                tLickOnset = cell2mat(cellfun(@(x) [zeros(1,posShift), histcounts2013(x.bothLickOnset, [0, x.tpmTime{plane}]), zeros(1,posShift)], v(tind)','uniformoutput',false));

                lLickOffset = cell2mat(cellfun(@(x) [zeros(1,posShift), histcounts2013(x.leftLickOffset, [0, x.tpmTime{plane}]), zeros(1,posShift)], v(tind)','uniformoutput',false));
                rLickOffset = cell2mat(cellfun(@(x) [zeros(1,posShift), histcounts2013(x.rightLickOffset, [0, x.tpmTime{plane}]), zeros(1,posShift)], v(tind)','uniformoutput',false));
                tLickOffset = cell2mat(cellfun(@(x) [zeros(1,posShift), histcounts2013(x.bothLickOffset, [0, x.tpmTime{plane}]), zeros(1,posShift)], v(tind)','uniformoutput',false));
        ...
        %     cellnum = 1;    
            ...
            cv = cvglmnet(input, spk, 'poisson', glmnetOpt,[],[],[],true);
        ...
        %     spkTest = spkTest(finiteInd)';
            ...
            if devianceFullNull > chi2inv(1-pThresholdNull, dfFullNull)
                ...
                for pi = 1 : 5
                    tempTrainInput = trainingInputMat{planeInd}(:,setdiff(1:size(trainingInputMat{planeInd},2),indPartial{pi}));
                    tempTestInput = testInputMat{planeInd}(:,setdiff(1:size(testInputMat{planeInd},2),indPartial{pi}));
                    cvPartial = cvglmnet(tempTrainInput(finiteInd,:), spk, 'poisson', glmnetOpt,[],[],[],true);
                    iLambda = find(cvPartial.lambda == cvPartial.lambda_1se);
                    partialTouchLL = nansum(log(poisspdf(spkTest', exp([ones(size(tempTestInput,1),1), tempTestInput] * [cvPartial.glmnet_fit.a0(iLambda); cvPartial.glmnet_fit.beta(:,iLambda)]))));
                    devianceFullPartial = 2*(fullLogLikelihood - partialTouchLL);
