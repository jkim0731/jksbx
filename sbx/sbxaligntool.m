...
if(exist(fref,'file'))
    ...
    ref = squeeze(double(intmax('uint16'))*size(mm0.Data.img,4)-sum(mm0.Data.img,4,'double'))';
    clear('mm0');
    handles.status.String = sprintf('File %s has been loaded',fname);
    
    save(fref,'ref'); % save reference image
end
...
global fname ref running FileName xidx yidx
...
%     
    ...
    for p = 1:npass
        ...
        for i = 1:bsz:(max_idx)
            ...
            z = squeeze(double(intmax('uint16'))-double(squeeze(mm0.Data.img(handles.chan.Value,:,:,i:(i+M-1)))));
            z = permute(z,[2 1 3]);
            ...
            for(j=1:M)
                T(i+j-1,:) = t{j};
                reg = circshift(squeeze(z(:,:,j)),T(i+j-1,:));
                m = m + double(reg);
                mm.Data.img(handles.chan.Value,:,:,i+j-1) = intmax('uint16')-uint16(reg');
            end
