...
ops0                                = jk_build_ops3(db, ops0); % after this, db = ops0
...
if length(ops0.session) > 1
    opath = sprintf('%s/regops_%s_%s_%s.mat', ops0.ResultsSavePath, ops0.mouse_name, ops0.session(1), ops0.session(end));
else
    opath = sprintf('%s/regops_%s_%s.mat', ops0.ResultsSavePath, ops0.mouse_name, ops0.session);
end
