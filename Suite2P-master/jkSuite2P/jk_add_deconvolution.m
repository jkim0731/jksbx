...
function jk_add_deconvolution(ops, db)
ops = jk_build_ops3(db, ops);
ops0 = ops;
...
for i =  1:length(ops.planesToProcess)    
    ...
    fpath = sprintf('%s/F_%s_%03d_plane%d_proc.mat', ops.ResultsSavePath, ...
        ops.mouse_name, ops.session, iplane); 
    if exist(fpath, 'file')
        load(fpath);
    else
        fpath = sprintf('%s/F_%s_%03d_plane%d.mat', ops.ResultsSavePath, ...
            ops.mouse_name, ops.session, iplane); 
        dat = load(fpath);
    end
