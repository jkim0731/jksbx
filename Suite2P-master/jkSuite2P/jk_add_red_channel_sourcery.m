function jk_add_red_channel_sourcery(ops)

load(fullfile(ops.ResultsSavePath,'redchannel.mat'));

root = ops.ResultsSavePath;
fregops =  sprintf('regops_%s_%03d.mat', ops.mouse_name, ops.session);
flag = 0;
...
for i = 1:length(ops.planesToProcess)
    ...
    fname = sprintf('%s/F_%s_%03d_plane%d.mat', ops.ResultsSavePath, ops.mouse_name, ops.session, iplane);
    dat = load(fname);
    ...
    if flag 
        ...
        fname = sprintf('%s/regops_%s_%03d_plane%d.mat', ops.ResultsSavePath, ops.mouse_name, ops.session, iplane);
        dd = load(fname);
