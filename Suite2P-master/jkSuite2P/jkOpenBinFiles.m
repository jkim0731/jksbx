...
function [ops1, fid, fidRED, fidIntpol] = jkOpenBinFiles(ops, ops1, red_binary)
...
for i = 1:numPlanes
    for j = 1:size(ops1,2)
        ops1{i,j}.RegFile = fullfile(ops.RegFileRoot, ...
            sprintf('%s_%03d_plane%d.bin', ops.mouse_name, ops.session, i + (j-1)*numPlanes));
        ...
        if red_binary
            ops1{i,j}.RegFile2 = fullfile(ops.RegFileRoot, ...
                sprintf('%s_%03d_plane%d_RED.bin', ops.mouse_name, ops.session, i + (j-1)*numPlanes));
        ...
        if ops.interpolateAcrossPlanes && ~isempty(ops.RegFileBinLocation)
            ...
            folder = fullfile(ops1{i,j}.RegFileBinLocation, ops1{i,j}.mouse_name, ...
                ops1{i,j}.session, str, 'interpolated');
            if ~exist(folder, 'dir')
                mkdir(folder);
            end
            fidIntpol{i,j} = fopen(fullfile(folder, ...
                sprintf('%s_%03d_plane%d.bin', ops.mouse_name, ops.session, i + (j-1)*numPlanes)), 'w');
        end
    end
end