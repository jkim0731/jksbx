...
%     % Same length as spk. 
%
%     - pTouchOnset: protraction touch onset frames (binary)
%     - rTouchOnset: retraction touch onset frames (binary)
%           Most of the touch bouts are similar or less than a single frame, so
%               it's hard to distinguish touch onset or offset cells in TPM
%     
%
...
%     - whiskingFrames: whisking onset (binary)
%           Simiar to touch, whisking bouts are less than a single frame
%
%
%     - lLick: left licks (binary)
%     - rLick: right licks (binary)
%     
...
%         for cellnum = 1
        %     cellnum = 1;    
            ...
            cv = cvglmnet(input, spk, 'binomial', glmnetOpt,[],[],[],true);
        
%             %% For deviance calculation check
%             nullp = mean(spk); % null binomial parameter. 
%             nullLogLikelihood = sum(log(binopdf(   spk,   ones(length(spk),1),   nullp*ones(length(spk),1)   )));
%             mu = exp([ones(size(input,1),1),input]*[cv.glmnet_fit.a0(iLambda); cv.glmnet_fit.beta(:,iLambda)]);
%             fullp = mu./(1+mu);
%             fullLogLikelihood = sum(log(binopdf(   spk,  ones(length(spk),1),  fullp   )));
%             saturatedLogLikelihood = 0;
%             devianceFullNull = 2*(fullLogLikelihood - nullLogLikelihood);
% 
%             dev0 = -2 * nullLogLikelihood;
%         
%             1 - fullLogLikelihood / nullLogLikelihood
        ...
        %     spkTest = spkTest';
            ...
            spkTest(spkTest > 0) = 1;
            testInput = testInputMat{planeInd}(finiteInd,:);
            %% (1) if the full model is significant
%             fitResult = zeros(1,6);

            nullp = mean(spkTest); % null binomial parameter. 
            nullLogLikelihood = sum(log(binopdf(   spkTest,   ones(length(spkTest),1),   nullp*ones(length(spkTest),1)   )));
            mu = exp([ones(size(testInput,1),1),testInput]*[cv.glmnet_fit.a0(iLambda); cv.glmnet_fit.beta(:,iLambda)]);
            fullp = mu./(1+mu);
            fullLogLikelihood = sum(log(binopdf(   spkTest,  ones(length(spkTest),1),  fullp   )));
            saturatedLogLikelihood = 0;
            devianceFullNull = 2*(fullLogLikelihood - nullLogLikelihood);
            devExplained = 1 - 2*(saturatedLogLikelihood - fullLogLikelihood) / 2*(saturatedLogLikelihood - nullLogLikelihood);
...
%                         tempTestInput = testInput(:,setdiff(1:size(testInput,2),indPartial{pi}));
