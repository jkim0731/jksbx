...
%     for si = 2
        ...
        savefnResult = sprintf('glmResponseType_JK%03dS%02d_glmnet_m15',mouse, session); % m(n) meaining method(n)
...
%                         touchMat = [tTouchCountMat, pTouchCountMat, rTouchCountMat, tTouchFramesMat, pTouchFramesMat, rTouchFramesMat, tTouchDurationMat, pTouchDurationMat, rTouchDurationMat];
                        ...
                        trainingInputMat{(cgi-1)*4 + plane} = single(allPredictors{(cgi-1)*4 + plane}(find(trainingPredictorInd{(cgi-1)*4 + plane}),:));
                        testInputMat{(cgi-1)*4 + plane} = single(allPredictors{(cgi-1)*4 + plane}(find(testPredictorInd{(cgi-1)*4 + plane}),:));
                    end
                end
...
%             rtest(ri).devExplained = zeros(length(u.cellNums),1);            
            ...
            cellTime = zeros(length(u.cellNums),1);
...
%             rtest(ri).cvDev = cvDev;
            runTime = toc(startTic);
            save(savefnResult, 'fit*', 'allPredictors', '*InputMat', 'indPartial', '*Group', '*Tn', 'lambdaCV', '*Opt', 'done', 'pThreshold*', '*Shift', 'runTime', 'cellTime');

%         end % of ri. random group selection index
        push_myphone(sprintf('single type Lasso GLM done for JK%03d S%02d', mouse, session))
